                    ...
                    with st.expander(f"[{row['상태']}] {row['제목']} (기안:{row['이름']})"):
                        ...
                        for i, aid in enumerate(approver_ids):
                            ...
                            <div style='border: 1px solid #333; width: 70px; text-align: center; margin-left: -1px;'>
                                ...
                                <div style='padding: 8px 2px; font-weight: bold; font-size: 12px; color: black;'>{name}</div>
                        ...
                        <div style="border: 2px solid #000; padding: 40px; background-color: #fff; color: #000;">
                            {stamp_html}
                            <h1 style="text-align: center; text-decoration: underline; margin-bottom: 30px; color: black;">{row['결재유형']}</h1>
                            <table style="width: 100%; border-collapse: collapse; border: 1px solid #000; color: black;">
                                <tr>
                            ...
                            <p style="text-align: center; margin-top: 40px; font-size: 14px; color: black;">위와 같이 기안하오니 승인하여 주시기 바랍니다.</p>
                            <p style="text-align: center; font-weight: bold; margin-top: 20px; color: black;">{datetime.now().strftime('%Y년 %m월 %d일')}</p>
                        </div>
                        """
                        ...
                        if st.button("📄 기안서 출력/PDF 저장", key=f"prt_{row['결재ID']}"):
                            ...
                            pwin.document.write('<html><head><title>HR 기안서</title></head><body>');
                        ...
                        if can_approve:
                            ...
                            if c1.button("✅ 승인", key=f"ok_{row['결재ID']}"):
                                ...
                                if next_stat == "승인" and "연차" in row['결재유형']:
                                    ...
                                    if d_match:
                                        db.worksheet("Schedules").append_row([str(u['사업자번호']), d_match.group(), row['이름'], f"[연차] {row['제목']}"])
...
# --- 4. 메인 대시보드 ---
    ...
    if "홈" in menu:
                                    ...
                                    with st.popover(s['이름'], use_container_width=True):
                                        with st.form(f"fm_{s['이름']}_{day}"):
                                            ...
                                            if st.form_submit_button("최종 저장"):
                                                if rs:
                                                    fi, fo = smart_time_parser(ni, 0), smart_time_parser(no, 0)
        ...
        if st.button("📄 엑셀 생성"):
            mr = recs[recs['사업자번호'].astype(str) == str(u['사업자번호'])].copy()
            output = io.BytesIO()
