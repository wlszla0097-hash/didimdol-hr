...
def get_engine():
    ...
    # 2. Streamlit Secrets에서 보안 정보 읽기 (클라우드 배포용 핵심)
    # 'gcp_service_account'는 나중에 Streamlit 설정창에 입력할 이름과 일치해야 합니다.
    creds_info = st.secrets["gcp_service_account"]
    
    # 3. JSON 파일 대신 딕셔너리(dict) 데이터로 인증
                ...
                for _, row in display_df.iterrows():
                    ...
                    with st.expander(f"[{row['상태']}] {row['제목']} (기안:{row['이름']})"):
                        ...
                        for i, aid in enumerate(approver_ids):
                            ...
                            stamp_html += f"""
                            <div style='border: 1px solid #333; width: 70px; text-align: center; margin-left: -1px;'>
                                <div style='background: #f8f9fa; border-bottom: 1px solid #333; font-size: 10px; padding: 2px;'>{i+1}차 결재</div>
                                <div style='padding: 8px 2px; font-weight: bold; font-size: 12px;'>{name}</div>
                                <div style='border-top: 1px dotted #ccc; color: #d9534f; font-size: 9px; padding: 2px;'>{s_text}</div>
                            </div>"""
                        stamp_html += "</div>"

                        # --- [UI] 정식 격자 양식 ---
                        doc_body = f"""
                        <div style="border: 2px solid #000; padding: 40px; background-color: #fff; color: #000;">
                            {stamp_html}
                            <h1 style="text-align: center; text-decoration: underline; margin-bottom: 30px;">{row['결재유형']}</h1>
                            <table style="width: 100%; border-collapse: collapse; border: 1px solid #000;">
                                <tr>
                                    <td style="border: 1px solid #000; padding: 10px; width: 15%; background: #f2f2f2; font-weight:bold;">기안자</td>
                                    <td style="border: 1px solid #000; padding: 10px; width: 35%;">{row['이름']}</td>
                                    <td style="border: 1px solid #000; padding: 10px; width: 15%; background: #f2f2f2; font-weight:bold;">기안일시</td>
                                    <td style="border: 1px solid #000; padding: 10px; width: 35%;">{row['기안일시']}</td>
                                </tr>
                                <tr>
                                    <td style="border: 1px solid #000; padding: 10px; background: #f2f2f2; font-weight:bold;">제목</td>
                                    <td colspan="3" style="border: 1px solid #000; padding: 10px;">{row['제목']}</td>
                                </tr>
                                <tr>
                                    <td colspan="4" style="border: 1px solid #000; padding: 30px; height: 250px; vertical-align: top; line-height: 1.6;">
                                        <b>[기안 내용]</b><br><br>{row['내용'].replace('|', '<br>')}
                                    </td>
                                </tr>
                            </table>
                            <p style="text-align: center; margin-top: 40px; font-size: 14px;">위와 같이 기안하오니 승인하여 주시기 바랍니다.</p>
                            <p style="text-align: center; font-weight: bold; margin-top: 20px;">{datetime.now().strftime('%Y년 %m월 %d일')}</p>
                        </div>
                        """
                        ...
                        if st.button("📄 기안서 출력/PDF 저장", key=f"prt_{row['결재ID']}"):
                            ...
                            pwin.document.write('<html><head><title>Didimdol HR 기안서</title></head><body>');
                            pwin.document.write('{doc_body.replace(chr(10), "").replace("'", "\\'")}');
                        ...
                        if can_approve:
                            ...
                            if c1.button("✅ 승인", key=f"ok_{row['결재ID']}"):
                                sheet4.update_cell(actual_row, 8, next_stat)
                                sheet4.update_cell(actual_row, 10, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                                # [일정 연동] 최종 승인일 때만 홈 달력 반영
                                if next_stat == "승인" and "연차" in row['결재유형']:
                                    d_match = re.search(r'\d{4}-\d{2}-\d{2}', row['내용'])
                                    if d_match:
                                        db.get_worksheet(3).append_row([str(u['사업자번호']), d_match.group(), row['이름'], f"[연차] {row['제목']}"])
                                st.success("승인 완료!"); st.cache_data.clear(); st.rerun()
                            if c2.button("❌ 반려", key=f"no_{row['결재ID']}"):
                                sheet4.update_cell(actual_row, 8, "반려")
                                st.error("반려됨"); st.cache_data.clear(); st.rerun()
...
if st.session_state['user_info'] is None:
    ...
    with col_m:
        ...
        with t_j:
            with st.form("join"):
                st.write("##### 🏢 디딤돌HR 가입")
                j_b, j_c = st.text_input("사업자번호"), st.text_input("사업장명")
                j_i, j_p, j_n = st.text_input("ID"), st.text_input("PW", type="password"), st.text_input("성함")
                if st.form_submit_button("가입신청"):
                    get_engine().get_worksheet(1).append_row([j_b, j_c, j_i, j_p, j_n, 'Manager', '8', '스타터', '정규직', '40'])
                    st.success("완료")
...
# --- 4. 메인 대시보드 ---
    ...
    db = get_engine()
    st.sidebar.markdown(logo_html if logo_html else "### DIDIMDOL", unsafe_allow_html=True)
    ...
    recs = fetch(0)
    today_dt = date.today()
    my_t = recs[(recs.get('아이디','') == str(u.get('아이디',''))) & (recs.get('일시','').str.contains(today_dt.strftime("%Y-%m-%d")))] if not recs.empty else pd.DataFrame()
                    ...
                    with cols[i]:
                        ...
                        if not sch.empty:
                            ...
                            for _, row in ds.iterrows():
                                with st.popover(row['이름'], use_container_width=True):
                                    st.write(f"📌 {row['내용']}")
                else: cols[i].write("")
                                        ...
                                        with st.form(f"fm_{s['이름']}_{day}"):
                                            ...
                                            if st.form_submit_button("최종 저장"):
                                                if rs:
                                                    ...
                                                    if in_m.any(): sheet0.update_cell(df_t.index[in_m][0]+2, 4, f"{d_str} {fi}")
                                                    else: sheet0.append_row([str(u['사업자번호']), s['아이디'], s['이름'], f"{d_str} {fi}", "출근(수정)", rs, ""])
                                                    st.success("저장됨"); st.cache_data.clear(); st.rerun()
                else: cols[i].write("")

    elif menu == "👥 직원 관리":
        st.header("👥 직원 정보 관리")
        ms = fetch(1)[fetch(1).get('사업자번호','').astype(str) == str(u['사업자번호'])]
        if not ms.empty:
            st.dataframe(ms[['이름', '아이디', '권한']], use_container_width=True, hide_index=True)
            t = st.selectbox("수정 직원", ["선택"] + ms['이름'].tolist())
            if t != "선택":
                td = ms[ms['이름'] == t].iloc[0]
                with st.form("ed"):
                    en = st.text_input("성함", value=td.get('이름',''))
                    if st.form_submit_button("업데이트"): st.success("완료")

    elif menu == "📂 데이터 추출":
        st.header("📂 증빙 데이터 최종본 추출")
        if st.button("📄 엑셀 생성"):
            mr = recs[recs.get('사업자번호','').astype(str) == str(u['사업자번호'])].copy()
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                mr.to_excel(writer, index=False, sheet_name='근태')
                fetch(3)[fetch(3).get('사업자번호','').astype(str) == str(u['사업자번호'])].to_excel(writer, index=False, sheet_name='일정')
